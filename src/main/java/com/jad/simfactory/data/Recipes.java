...
class Recipes extends FactoryData<Recipe> {
    ...
    protected void generateDataSet() {
        for (Product product : this.products) {
            ...
            for (int i = 0; i < Action.getNbProducts(recipe.getAction()); i++) {
                Product component = this.getRandomGoodProduct(product);
                if (component != null) {
                    recipe.addProduct(component);
                }
            }
            if (Action.getNbProducts(recipe.getAction()) == recipe.getProducts().size()) {
    ...
    private static final int NbMaxTries = 2;
